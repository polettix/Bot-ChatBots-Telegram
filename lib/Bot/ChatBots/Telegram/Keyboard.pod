...
=head2 B<< keyboard >>
   ...
   my $keyboard = keyboard(@keyboard_rows); # OR
      $keyboard = keyboard(\@keyboard_rows);
...
=head2 B<< new >>
   ...
   my $kbd = Bot::ChatBots::Telegram::Keyboard->new(keyboard => \@aref);

Constructor. It accepts a C<keyboard> parameter, that must be an array of
arrays, each of which represents a row.

The format of each row is exactly that of a regular row in a Telegram
keyboard, i.e. it must contain one or more L<Keyboard
Buttons|https://core.telegram.org/bots/api/#keyboardbutton>. These buttons
are hash references exactly as stated in Telegram documentation, with the
addition of a C<_value> parameter that holds the I<real> value to
associate to the button (which can be retrieved via L</get_value>).
...
=item C<< invalid empty keyboard >>

the input keyboard parameter provided to L</new> is empty... and this is
not acceptable!
